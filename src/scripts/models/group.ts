...
export function createSourceGroup(name: string): AppThunk<number> {
    return (dispatch, getState) => {
        ...
        const group = new SourceGroup([], name)
...
function outlineToSource(outline: Element): [ReturnType<typeof addSource>, string] {
    ...
    const name = outline.getAttribute("text") || outline.getAttribute("title")
    if (url) {
    ...
    return async dispatch => {
        ...
        window.utils.showOpenDialog(filters).then(data => {
            if (data) {
                ...
                // Error tuple: [source name, error object]
                const errors: [string, Error | unknown][] = []
                ...
                const promises = sources.map(([s, gid, url]) => {
                    return dispatch(s)
                        .then(sid => {
...
export function exportOPML(): AppThunk {
    return (_, getState) => {
        ...
        window.utils.showSaveDialog(filters, "*/Simple_Reader_Export.opml").then(write => {
            if (write) {
                const state = getState()
                const xml = domParser.parseFromString(
                    '<?xml version="1.0" encoding="UTF-8"?><opml version="1.0"><head><title>Simple Reader Export</title></head><body></body></opml>',
                    "text/xml"
                )
