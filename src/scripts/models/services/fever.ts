...
export const feverServiceHooks: ServiceHooks = {
    ...
    updateSources: () => async (dispatch, getState) => {
        ...
        const feeds: FeverFeed[] = response.feeds
        const feedGroups: FeverFeedGroup[] = response.feeds_groups
        if (feeds === undefined) throw APIError()
        let groupsMap: Map<string, string>
        if (configs.importGroups) {
            // Import groups on the first sync
            const groups: FeverGroup[] = (await fetchAPI(configs, "&groups")).groups
            ...
            for (const group of feedGroups) {
                for (const fid of group.feed_ids.split(",")) {
        ...
        if (items.length > 0) {
            ...
            const parsedItems = items.map(i => {
                ...
                if (img && img.src) {
                    ...
                    const a = dom.querySelector("body>ul>li:first-child>a") as HTMLAnchorElement
