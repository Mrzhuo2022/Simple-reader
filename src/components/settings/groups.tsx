...
type GroupsTabState = {
    ...
    selectedSources: RSSSource[] | null
...
class GroupsTab extends React.Component<GroupsTabProps, GroupsTabState> {
    ...
    reorderGroups = (item: SourceGroup) => {
        const draggedItem = this.groupSelection.isIndexSelected(this.groupDraggedIndex)
            ? (this.groupSelection.getSelection()[0] as SourceGroup)
            : this.groupDraggedItem!

        const insertIndex = item.index
        const groups = this.props.groups.filter(g => g.index != draggedItem.index)
    ...
    reorderSources = (item: RSSSource) => {
        const draggedItems = this.sourcesSelection.isIndexSelected(this.sourcesDraggedIndex)
        ...
        const items = this.state.selectedGroup.sids.filter(sid => !draggedItems.includes(sid))
    ...
    handleInputChange = (event: React.FormEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        } else if (name === "editGroupName") {
            this.setState({ editGroupName: value })
        }
    }
