...
async function fetchAll(configs: GReaderConfigs, params: string): Promise<Set<string>> {
    ...
    let fetched: GReaderItemRef[] | undefined
...
export const gReaderServiceHooks: ServiceHooks = {
    ...
    updateSources: () => async (dispatch, getState) => {
        ...
        const subscriptions: GReaderSubscription[] = (await response.json()).subscriptions
    ...
    fetchItems: () => async (_, getState) => {
        ...
        const items = new Array<GReaderFetchedItems["items"][0]>()
        let fetchedItems: GReaderFetchedItems["items"]
        let continuation: string | undefined
        do {
            try {
